...
contract InheritanceDollarVaultUpgradeableV2 is
    Initializable,
    UUPSUpgradeable,
    ...
    IERC20Metadata
{
    ...
    function migrateLotsFor(address user, uint256 maxSteps)
        external
        returns (uint256 migrated, uint256 cursor, uint256 len)
    {
        return _migrateLotsLazy(user, maxSteps);
    }

    function migrateMyLots(uint256 maxSteps) external returns (uint256 migrated, uint256 cursor, uint256 len) {
        return _migrateLotsLazy(msg.sender, maxSteps);
    }

    function _migrateLotsLazy(address user, uint256 maxSteps)
        internal
        returns (uint256 migrated, uint256 cursor, uint256 len)
    {
        uint256 i = _migrateCursor[user];
        len = _lots[user].length;

        if (i < _head[user]) i = _head[user];

        uint256 start = i;
        uint256 steps = 0;

        while (i < len && steps < maxSteps) {
            if (_lots[user][i].amount > 0) {
                _attachLotToBucket(user, i);
                unchecked {
                    ++migrated;
                }
            }
        ...
        _migrateCursor[user] = i;
        cursor = i;

        if (i != start) emit LotsMigratedToBuckets(user, start, i);
    }
    ...
    function bucketDebugTotals(address user)
        external
        view
        returns (
            uint256 bucketTotal,
        ...
        )
    {
        ...
        while (bucketId != 0) {
            ...
            while (lotNode != 0) {
                ...
                linkedLotTotal += lot.amount;

                if (block.timestamp >= lot.unlockTime) {
                    spendable += lot.amount;
                } else {
                    locked += lot.amount;
                }

                lotNode = _nextLotNode[user][lotNode];
            }

            bucketId = b.nextBucket;
        }
    }

    function bucketDebugCursor(address user) external view returns (uint256 cursor, uint256 len) {
        cursor = _migrateCursor[user];
        len = _lots[user].length;
    }
    ...
    function _shiftByYears(uint64 baseTs, uint16 deltaYears) internal pure returns (uint64) {
        ...
        // casting to uint64 is safe because shifted is a unix timestamp derived from Gregorian.yearStartTs/yearEndTs
    ...
    mapping(address => uint256) private _migrateCursor;
    ...
    event LotsMigratedToBuckets(address indexed user, uint256 fromIndex, uint256 toIndex);
